...
class List extends BaseParent {
    ...
    insert(index, item) {
        ...
        if (before) {
            this.changeObservable.dispatch({ id: this._options.generateId(), type: mutations_1.MutationType.INSERT, beforeId: before.id, value: item, targetId: this.id, targetPath: this.getPath() });
        }
        else {
            this.changeObservable.dispatch({ id: this._options.generateId(), type: mutations_1.MutationType.INSERT, beforeId: null, targetId: this.id, value: item, targetPath: this.getPath() });
        }
    }
    replace(index, item) {
        const oldItem = this._items[index];
        this._items.splice(index, 1, item);
        this.changeObservable.dispatch({ id: this._options.generateId(), type: mutations_1.MutationType.REPLACE_LIST_ITEM, itemId: oldItem.id, targetId: this.id, value: item, targetPath: this.getPath() });
    }
    indexOf(item) {
        return this._items.indexOf(item);
    }
    ...
    move(oldIndex, newIndex) {
        ...
        this.changeObservable.dispatch({ id: this._options.generateId(), type: mutations_1.MutationType.MOVE_LIST_ITEM, itemId: item.id, targetId: this.id, beforeId: before && before.id, targetPath: this.getPath() });
    }
    removeAt(index) {
        ...
        this.changeObservable.dispatch({ id: this._options.generateId(), type: mutations_1.MutationType.DELETE, targetId: item.id, targetPath: itemPath });
    }
    traverse(handler) {
...
class Map extends BaseParent {
    ...
    setValue(propertyName, value) {
        ...
        if (value == null) {
            ...
            this.changeObservable.dispatch({ id: this._options.generateId(), type: mutations_1.MutationType.MAP_UNSET, oldValueId: oldItem && oldItem.id, targetId: this.id, propertyName, targetPath: this.getPath() });
        }
        else {
            this._properties[propertyName] = value;
            this.changeObservable.dispatch({ id: this._options.generateId(), type: mutations_1.MutationType.MAP_SET, targetId: this.id, propertyName, value, targetPath: this.getPath() });
        }
    }
    remove(value) {
