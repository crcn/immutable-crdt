...
var Document = /** @class */ (function () {
    ...
    Document.prototype.updateState = function (newState) {
        ...
        var ots = immutable_ot_1.diff(this._currentState, newState, { adapter: this._adapter });
     ...
     */
    Document.initialize = function (initialState, adapter) {
        if (adapter === void 0) { adapter = records_1.defaultAdapter; }
        var createRecord = records_1.$recordCreator(utils_1.generateId, adapter);
        var doc = new Document(adapter);
        doc._construct(createRecord(initialState || {}), createRecord);
        return doc;
    };
    /**
     */
    Document.deserialize = function (data, adapter) {
        if (adapter === void 0) { adapter = records_1.defaultAdapter; }
        var record = records_1.$deserializeRecord(data, adapter);
        var doc = new Document(adapter);
